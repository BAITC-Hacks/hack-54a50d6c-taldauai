...
**На Windows и WSL2 проект в этой проверке не запускался.** Ниже — инструкция по фактическим интерфейсам проекта, а не утверждение успешной Windows-приёмки. При финальной интеграции Windows-путь Vite alias исправлен через `fileURLToPath`. Это минимальная переносимая правка, но нативный Windows и совместимость ML всё ещё требуют собственного smoke test. WSL2 также не проверялся.
...
Если `@/api` не разрешается, проверьте наличие `src/api.ts` и актуальность `vite.config.ts`; не скрывайте Vite overlay.
