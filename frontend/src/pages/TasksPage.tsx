...
export function TasksPage() {
  ...
  const saveReminder = async () => {
    ...
    try {
      ...
      setReminding(null)
      toast('Напоминание зафиксировано', 'Отметка сохранена в поручении.')
  ...
  return <div className="td-page">
    ...
    <div className="td-task-list">
      ...
      {!loading && filtered.map((task) => <article key={task.id} className={`td-task-row ${task.status === 'done' ? 'td-task-complete' : ''}`}>
        <button type="button" className="td-task-toggle" aria-label={task.status === 'done' ? 'Вернуть поручение в работу' : 'Отметить поручение выполненным'} aria-pressed={task.status === 'done'} onClick={() => markDone(task)} disabled={task.needs_review}>
