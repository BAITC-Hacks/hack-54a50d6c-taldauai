...
function pageTitle(pathname: string) {
  ...
  if (pathname === '/notifications') return 'Напоминания'
  ...
  return <div className="min-h-screen bg-white text-[#171717]">
    ...
    <aside className={cn(
      ...
      <nav className="flex flex-col gap-[5px]" aria-label="Основная навигация">
        ...
        >
          <Icon className="h-[19px] w-[19px]" strokeWidth={1.7} /> {label}{to === '/notifications' && unread > 0 && <span aria-label={`Непрочитанных напоминаний: ${unread}`} className="ml-auto rounded-full bg-red-600 px-1.5 text-xs text-white">{unread}</span>}
        </NavLink>)}
      </nav>
