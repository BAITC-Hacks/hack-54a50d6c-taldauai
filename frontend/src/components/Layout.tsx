...
import { Bell, AudioLines, CalendarDays, FileAudio, ListChecks, ShieldCheck, Upload } from 'lucide-react'
...
function NavItems({ compact = false, unread = 0 }: { compact?: boolean; unread?: number }) {
  ...
  return <nav className={cn('flex', compact ? 'justify-around' : 'flex-col gap-1')} aria-label="Основная навигация">
    ...
    >
      ...
      <span className={compact ? 'max-w-full truncate' : ''}>{label}</span>{to === '/notifications' && unread > 0 && <span aria-label={`Непрочитанных напоминаний: ${unread}`} className="rounded-full bg-red-600 px-1.5 text-xs text-white">{unread}</span>}
    </NavLink>)}
  </nav>
}
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
    <aside className="sticky top-0 hidden h-screen flex-col border-r bg-white px-3 py-4 lg:flex">
      ...
      <div className="mt-auto border-t px-2 pt-4">
        <div className="flex items-start gap-2.5 text-xs leading-5 text-slate-500">
          ...
          <span><strong className="font-medium text-slate-700">Локальная обработка</strong><br />Аудио обрабатывается локальными моделями.</span>
        </div>
      </div>
    </aside>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] pt-1 backdrop-blur lg:hidden">
      <NavItems compact unread={unread} />
    </div>
  </div>
}
