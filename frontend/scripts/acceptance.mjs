...
try {
  ...
  const manual = page.locator('tbody tr').filter({ hasText: 'Ручное поручение:' })
  await manual.getByText('Требует проверки', { exact: true }).waitFor()
  await manual.getByRole('button', { name: 'Подтвердить', exact: true }).click()
  await manual.getByText('Проверено', { exact: true }).waitFor()
  const downloadPromise = page.waitForEvent('download')
  await page.getByRole('button', { name: 'Скачать протокол (DOCX)' }).click()
