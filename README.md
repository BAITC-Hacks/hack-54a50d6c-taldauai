...
После проверки извлечённых поручений локальная модель отдельно формирует тему встречи, обсуждённые факты, решения и открытые вопросы — только для разделов с опорой на транскрипт. Для каждого пункта проверяется дословная цитата в указанных исходных сегментах; это проверка источника, а не гарантия правильного толкования. Исполнители и сроки добавляются из текущего списка поручений. Длинный текст обрабатывается частями. При сбое модели остаются отмеченные выдержки и предупреждение; результат совещания не теряется.

Изменение ручных поручений не пересоздаёт саммари автоматически. Для уже сохранённого совещания можно обновить только саммари без повторного ASR/диаризации (из активного backend-окружения с настроенным `.env`):

```bash
cd backend
python -m app.refresh_summary MEETING_ID
```

Команда учитывает текущие ручные правки, сохраняет исходный ML JSON и отклоняет сохранение, если версии поручений изменились во время генерации.

По умолчанию саммари использует `TALDAU_LLM_MODEL`. Можно отдельно задать `TALDAU_SUMMARY_MODEL=qwen2.5:14b` для установленной локальной модели; это требует больше памяти и времени. Проверка `ml.doctor` проверяет её наличие, если переменная задана.
